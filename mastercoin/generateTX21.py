...
cid_sale = listOptions['currency_id_sale']
amt_sale = int(listOptions['amount_for_sale']*1e8)
cid_desired = listOptions['currency_id_des']
amt_desired = int(listOptions['amount_des']*1e8)
...
cid_sale_b = hex(cid_sale)[2:].rjust(8,"0")              # 4 bytes
amt_sale_b = hex(amt_sale)[2:].rjust(16,"0")         # 8 bytes
cid_desired_b = hex(cid_desired)[2:].rjust(8,"0")        # 4 byte
amt_desired_b = hex(amt_desired)[2:].rjust(16,"0") # 8 byte
action = hex(action)[2:].rjust(2,"0") # 1 byte
...
print fee_total, available_balance , largest_spendable_input
...
if testnet:
    validnextoutputs = { "mpexoDuSkGGqvqrkrjiFng38QPkJQVFyqv": 0.000055 }
else:
