...
if sys.argv[1] == '-ui':
  import CLI_prompter
  listOptions = CLI_prompter.callCLI(54)
else:
  listOptions = { 
...
if sys.argv[1] == '-ui':
  ...
  if sys.argv[15] == 'local':
    conn = bitcoinrpc.connect_to_local()
  elif sys.argv[15] == 'remote':
    conndetails=open(sys.argv[16]).readline().split(':')
...
prev_prop_id_bytes = hex(previous_property_id)[2:].rstrip("L").rjust(8,"0")  # 4 bytes
